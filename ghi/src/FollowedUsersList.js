...
const FollowedUsersList = () => {
    ...
    const fetchFollowedAccounts = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_API_HOST}/followed-accounts/${account_id}`,
          {
            headers: {
          ...
          if (Array.isArray(followingIds)) {
            ...
            const accountsPromises = followingIds.map(async (followingId) => {
              const accountResponse = await fetch(
                `${process.env.REACT_APP_API_HOST}/api/account/${followingId}`,
                {
                  headers: {
                    Authorization: `Bearer ${authToken}`,
                  },
  ...
  const handleUnfollow = async (followingId) => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_HOST}/accounts/${account_id}/unfollow`,
        {
          method: "DELETE",
          headers: {
