...
const Account = ({ isAuthenticated, setIsAuthenticated }) => {
  ...
  const fetchTopRecentUploads = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_HOST}/api/random-recent-uploads`,
        {
          method: "GET",
          headers: {
  ...
  const fetchRandomLikedSongs = async (account_id) => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_HOST}/liked-songs/random/${account_id}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
        ...
        if (authToken) {
          ...
          if (account_id) {
            ...
            const userDataResponse = await fetch(
              `${process.env.REACT_APP_API_HOST}/account/${account_id}`
            );
            ...
            const userSongsResponse = await fetch(
              `${process.env.REACT_APP_API_HOST}/user-songs/${account_id}`
            );
  ...
  const handleSearchUser = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_HOST}/api/account?username=${searchUsername}`
      );
      if (response.ok) {
  ...
  return (
    <div className="profile">
      <div className="container">
        <img
          src={currentUser.banner_url || banner_url}
          alt="banner"
        ...
        <img
          src={currentUser.profile_picture_url || profile_picture_url}
          alt="Profile"
        ...
        <div className="shadow p-4 mt-4">
          <div className="link-container">
            <Link
              ...
              to={`/account/liked-songs/${account_id}`}
            >
              Liked Songs
            </Link>
            <Routes>
            ...
            <Link
              className="profile-link"
              to={`/followed-users-list/${account_id}`}
            >
              Following
            </Link>
            <Routes>
            ...
            <Link
              className="profile-link"
              to={`/account/all-songs/${account_id}`}
            >
              Your Songs
            </Link>
          </div>
          ...
          {searchedUserData && (
            <div>
              ...
              <Link to={`/account/liked-songs/${searchedUserData.account_id}`}>
                View Liked Songs
              </Link>
              <Link to={`/account/all-songs/${searchedUserData.account_id}`}>
                View Posted Songs
              </Link>
            </div>
          )}
